...
## 2024-06-26 视频流信息页面标定弹窗与多边形编辑优化

- 标定检测区域弹窗支持图片缩放、滚动，底部操作栏美化并左右分布。
- 支持多边形编辑：点击"编辑多边形"后可在图片上点击绘制多边形，双击结束，绿色线条和顶点高亮。
- 支持缩放（+/-）、清除、保存多边形。
- "查看检测区域"时弹窗底部操作栏自动隐藏，仅展示图片和多边形。
- 相关canvas绘制、事件、数据结构全部本地mock，便于后续扩展。
