<template>
  <div class="open-repo-root">
    ...
    <ElDialog
      ...
      :title="dialogMode === 'add' ? '新增信息' : '编辑信息'"
      width="500px"
      destroy-on-close
...
<script setup lang="ts">
              ...
              return h('div', { style: 'display: flex; justify-content: center;' }, [
                h('img', {
                  style:
                    'width: 50px; height: 50px; border-radius: 4px; cursor: pointer; object-fit: cover;',
                  src: row.openFeature,
                  onClick: () => handleImageClick(row.openFeature)
                })
              ])
            } else {
              return h('span', '暂无特征')
  ...
  const handleTabLabelEditComplete = (source?: string) => {
    ...
    // 找到对应的标签并更新
    const targetTab = openTabs.value.find((tab) => tab.name === editingTabName.value)
    if (targetTab) {
  ...
  const handleLoadOpenFeatures = () => {
    ...
    setTimeout(() => {
      ...
      activeTab.items.forEach((item) => {
        if (!item.openFeature) {
          item.openFeature = '/assets/facefeature.jpg'
          count++
        }
      })
      ...
      if (valid) {
        ...
        if (dialogMode.value === 'add') {
          ...
          const newItem: OpenItem = {
            ...
            name: openForm.name,
            desc: openForm.desc,
            image: openForm.image || '/assets/face.jpg',
            openFeature: '',
