<template>
  <div class="face-repo-root">
    <ArtTableFullScreen>
      <div class="face-repo-page" id="table-full-screen">
        <div class="layout-container">
          <!-- 左侧标签页导航 -->
          <div class="left-tabs">
            <div class="tabs-container">
              <div v-for="tab in faceTabs" :key="tab.name" class="tab-item"
              ...
              <ArtTable ref="tableRef" row-key="id" :data="tableData" :loading="loading"
                  ...
                  <div class="empty-data">
                    <el-empty :image-size="120" description="暂无数据">
                      <template #description>
                        <p>{{ !hasTabData ? '请先创建分类' : !activeTabName ? '请选择一个分类' : '当前分类下暂无人脸数据' }}</p>
                      </template>
                      <el-button v-if="hasTabData && activeTabName" type="primary" @click="handleAddFace">添加人脸</el-button>
                ...
                <template #default>
                  <template v-for="col in columns" :key="col.prop || col.type">
                    ...
                    <ElTableColumn v-else-if="col.prop === 'faceFeature'" :prop="col.prop" :label="col.label" :width="col.width">
                      <template #default="scope">
                        <div @click="handleImageClick(scope.row.faceFeature)" v-if="scope.row.faceFeature">
                          <el-image
                            style="width: 50px; height: 50px; border-radius: 4px; cursor: pointer;"
                            :src="scope.row.faceFeature"
                            fit="cover"
                          />
                        </div>
    ...
    <!-- 人脸新增/编辑弹窗 -->
    <ElDialog v-model="dialogVisible" :title="dialogMode === 'add' ? '新增人脸信息' : '编辑人脸信息'" width="500px" destroy-on-close
      ...
      <ElForm :model="faceForm" :rules="rules" ref="formRef" label-width="100px">
        ...
        <ElFormItem label="年龄" prop="age">
          <ElInputNumber v-model="faceForm.age" :min="1" :max="120" placeholder="请输入年龄" style="width: 100%" />
        </ElFormItem>
...
const formItems: SearchFormItem[] = [
  ...
  { label: '性别', prop: 'gender', type: 'select', config: { clearable: true, options: [
    { label: '男', value: '男' },
    { label: '女', value: '女' }
  ] } }
]
...
const { columnChecks, columns } = useCheckedColumns(() => [
  ...
  { prop: 'image', label: '人员照片', width: 80 },
  { prop: 'faceFeature', label: '特征图', width: 80 },
...
const confirmDeleteTab = (tab: TabItem) => {
  ...
  if (hasItems) {
    ElMessageBox.confirm(`当前分类有人脸信息正在使用，确认删除将影响这些数据。是否继续删除"${tab.label}"？`, '删除确认', {
...
const deleteTab = (tabName: string) => {
  ...
  const index = faceTabs.value.findIndex(tab => tab.name === tabName)
  if (index === -1) return

  // 获取待删除标签的数据
  const tabToDelete = faceTabs.value[index]
  ...
  if (faceTabs.value.length > 0) {
    // 如果还有标签，总是切换到第一个标签
    activeTabName.value = faceTabs.value[0].name
  } else {
    // 如果没有标签了，清空激活标签
    activeTabName.value = ''
  }
...
const startEditTabLabel = (tab: TabItem) => {
  ...
  if (hasItems) {
    ElMessageBox.confirm('当前分类有人脸信息，确认修改可能影响识别结果', '修改确认', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(() => {
...
const handleTabLabelEditComplete = () => {
  ...
  // 检查名称是否已存在
  const exists = faceTabs.value.some(tab =>
  ...
  // 更新标签名称
  const tabToUpdate = faceTabs.value.find(tab => tab.name === editingTab.value?.name)
  if (tabToUpdate) {
...
const handleFileChange = (file: UploadFile) => {
  ...
  if (file.raw) {
    ...
    file.url = localUrl
    // 保存到表单，使用本地URL预览
    faceForm.image = localUrl
    
    ElMessage.info(`已选择新照片"${file.name}"，点击确定后生效`)
  } else if (file.url) {
    // 已有图片（编辑模式）
    faceForm.image = file.url
  }
}
...
const handleAddFace = () => {
  if (!hasTabData.value) {
    ElMessageBox.confirm('当前没有可用的分类，是否创建一个新分类？', '提示', {
      ...
      faceTabs.value.push(defaultTab)
      activeTabName.value = newName
...
const handleEdit = (row: FaceItem) => {
  ...
  Object.assign(faceForm, {
    ...
    desc: row.desc,
    image: row.image || '',
    faceFeature: row.faceFeature || ''
  })

  // 如果有图片，添加到文件列表中进行回显
  if (row.image) {
    // 创建文件对象用于预览
    const fileName = row.id ? `face_${row.id.split('-').pop()}.jpg` : 'face.jpg'
    const fileItem = { 
      name: fileName,
      url: row.image,
      // 标记为已有文件，非新上传
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 检查是否有图片
    if (!faceForm.image && fileList.value.length === 0) {
      ElMessage.error('请上传人员照片')
      return
    }

    // 处理上传图片
    let imageUrl = faceForm.image
    ...
    const savingInstance = ElMessage.info({
      message: dialogMode.value === 'add' ? '正在添加人脸信息...' : '正在保存编辑内容...',
      duration: 0,
      showClose: true
    })
    ...
    try {
      ...
      if (dialogMode.value === 'add') {
        // 新增模式
        // 生成唯一ID
        const newId = generateUniqueId(`face-${activeTabName.value}`)
        ...
        const newFace = {
          ...
          desc: faceForm.desc,
          image: imageUrl,
          faceFeature: needUpload ? '/assets/face.jpg' : '', // 新上传图片才生成特征图
        ...
        if (currentTab) {
          ...
          currentTab.items.unshift(newFace)
          savingInstance.close()
          ElMessage.success('添加人脸信息成功')
        ...
        if (currentTab) {
          ...
          if (!currentTab.items) {
            currentTab.items = []
            savingInstance.close()
            ElMessage.error('找不到人脸数据，请重试')
            return
          }
          
          const idx = currentTab.items.findIndex(item => item.id === faceForm.id)
          if (idx >= 0) {
            // 获取旧数据，保留不变的字段
            ...
            currentTab.items[idx] = {
              ...
              faceFeature: needUpload ? '/assets/face.jpg' : oldData.faceFeature
            }
            
            savingInstance.close()
            ElMessage.success('编辑人脸信息成功')
            dialogVisible.value = false
            getTableData()
          } else {
            savingInstance.close()
            ElMessage.error('找不到要编辑的人脸信息，请重试')
          }
        } else {
          savingInstance.close()
          ElMessage.error('找不到当前分类，请重试')
...
function getTableData() {
  ...
  setTimeout(() => {
    ...
    // 确保items数组存在
    const items: FaceItem[] = currentTab.items || []
    ...
    if (formFilters.gender) {
      filtered = filtered.filter((item) => item.gender === formFilters.gender)
    }
...
const handleDelete = (row: FaceItem) => {
  ElMessageBox.confirm(`确定要删除"${row.name}"的人脸信息吗？`, '删除确认', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    ...
    if (currentTab && currentTab.items) {
      const idx = currentTab.items.findIndex((item: FaceItem) => item.id === row.id)
      if (idx >= 0) {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(`确认删除选中的 ${selectedRows.value.length} 条记录吗？`, '警告', {
    ...
    if (currentTab && currentTab.items) {
      ...
      currentTab.items = currentTab.items.filter((item: FaceItem) => !idsToDelete.includes(item.id))
...
const handleLoadFaceFeatures = () => {
  ...
  if (!currentTab || !currentTab.items || currentTab.items.length === 0) {
    ElMessage.warning('当前分类没有人脸数据')
    return
  }
  
  // 计算需要处理的人脸数量
  const totalFaces = currentTab.items.length
  const needProcessFaces = currentTab.items.filter((item: FaceItem) => !item.faceFeature).length
  
  if (needProcessFaces === 0) {
    ElMessage.success('所有人脸特征已经载入')
    return
  }

  ElMessageBox.confirm(`将为当前分类中${needProcessFaces}张人脸图片生成特征，是否继续？`, '载入特征', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'info'
  }).then(async () => {
    ...
    try {
      ...
      // 更新没有特征的人脸数据
      currentTab.items.forEach((item: FaceItem) => {
        if (!item.faceFeature) {
          item.faceFeature = '/assets/face.jpg'
        }
      })
      ...
      // 关闭处理提示
      loadingInstance.close()
      ElMessage.success(`成功载入${needProcessFaces}张人脸特征`)
    } catch (error) {
      loadingInstance.close()
...
onMounted(() => {
  // 初始检查是否有标签页数据
  if (!hasTabData.value) {
    ElMessage.info('人脸底库中没有分类数据，请先创建分类')
  } else if (faceTabs.value.every(tab => !tab.items || tab.items.length === 0)) {
    ElMessage.info('人脸底库中没有人脸数据，请添加人脸')
  }
  getTableData()
})
</script>
