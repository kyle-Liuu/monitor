      ...
      <div class="layout-container">
        ...
        <div class="right-content">
          ...
          <ElCard shadow="never" class="art-table-card">
            ...
            <ArtTable ref="tableRef" row-key="id" :data="tableData" :loading="loading" :pagination="paginationState"
              :columns="columns" :table-config="{ emptyHeight: '360px' }" :layout="{ marginTop: 10, showIndex: false }"
              @row:selection-change="handleSelectionChange" @pagination:size-change="handleSizeChange"
              ...
              <template #image="{ row }">
                <div @click="handleImageClick(row.image)">
                  <el-image style="width: 50px; height: 50px; border-radius: 4px; cursor: pointer;" :src="row.image"
                    fit="cover" />
                </div>
              </template>
              <!-- 特征图列 -->
              <template #faceFeature="{ row }">
                <div @click="handleImageClick(row.faceFeature)" v-if="row.faceFeature">
                  <el-image style="width: 50px; height: 50px; border-radius: 4px; cursor: pointer;"
                    :src="row.faceFeature" fit="cover" />
                </div>
