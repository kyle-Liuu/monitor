        ...
        <ArtTableHeader v-model:columns="columnChecks" @refresh="handleRefresh"
          ...
          <template #left>
            <div class="toolbar-left">
              ...
              <ArtExcelExport :data="tableData as any" :columns="exportColumns as any" filename="组织数据"
                :auto-index="true" button-text="导出" @export-success="handleExportSuccess" />
              <ElButton @click="toggleExpand" v-ripple>{{ isExpanded ? '收起' : '展开' }}</ElButton>
            </div>
          </template>
        </ArtTableHeader>
        <!-- 表格 -->
        <ArtTable row-key="id" :tree-props="{ children: 'children' }" ref="tableRef" :loading="isLoading"
          :data="tableData" :columns="columns" :table-config="{
            emptyHeight: '360px'
          }" :layout="{ marginTop: 16, showIndex: false }" @row:expand-change="handleExpandChange">
          <!-- 名称列 -->
          <template #name="{ row }">
