<template>
  <ArtTableFullScreen>
    <div class="streaminfo-page" id="table-full-screen">
        ...
        <ElForm ref="formRef" :model="formData" :rules="rules" label-width="100px">
          ...
          <ElFormItem label="检测">
            <div style="display: flex; align-items: center;">
              ...
              <span v-if="checkStatus !== ''"
                :style="{ marginLeft: '12px', color: checkStatus === '在线' ? '#67C23A' : '#F56C6C' }">
                {{ checkStatus }}
              </span>
            </div>
          </ElFormItem>
          ...
          <ElFormItem label="算法标签">
            <div style="display: flex; align-items: center; gap: 8px; width: 100%;">
              <ElSelect v-model="selectedAlgos" multiple placeholder="请选择算法" style="flex: 1;">
                <ElOption v-for="algo in algoOptions" :key="algo.value" :label="algo.label" :value="algo.value" />
              </ElSelect>
              <ElButton type="primary" @click="algoDialogVisible = true">选择算法</ElButton>
            </div>
          </ElFormItem>
          <!-- 动态算法配置卡片区 -->
          ...
          <ElTabs v-model="algoTab">
            <ElTabPane v-for="tab in algoTabs" :key="tab.name" :label="tab.label" :name="tab.name">
              <ElCheckboxGroup v-model="algoDialogChecked">
                <ElCheckbox v-for="item in tab.items" :key="item.value" :label="item.value">{{ item.label }}
                </ElCheckbox>
              </ElCheckboxGroup>
            </ElTabPane>
          </ElTabs>
      ...
      <ElDialog v-model="calibrateDialogVisible" width="900px" top="40px" append-to-body :show-close="true">
        <div style="padding: 0 8px;">
          ...
          <!-- 图片区域 -->
          <div
            style="background: #fff; text-align: center; border-radius: 8px; padding: 18px 0; box-shadow: 0 2px 8px #0001;">
            <img src="/assets/image.png" style="max-width: 95%; border-radius: 8px; box-shadow: 0 1px 6px #0002;" />
          </div>
          <!-- 底部按钮 -->
...
const formData = reactive<{ orgId: string; streamName: string; streamCode: string; protocol: string; description: string; disable: boolean }>(
  {
    orgId: '',
    streamName: '',
    streamCode: '',
...
const rules: FormRules = {
  ...
  checkStatus: [
    { required: true, message: '请先进行视频流检测', trigger: 'change' }
  ]
}
...
const { columnChecks, columns } = useCheckedColumns(() => [
  ...
  { prop: 'description', label: '描述', minWidth: width.value < 500 ? 220 : '' },
  {
    ...
    formatter: (row: StreamItem) => {
      const labels = row.algos.map(val => algoOptions.find(a => a.value === val)?.label || val)
      return h(
        'div',
        {},
        labels.map(label =>
          h(ElTag, { style: 'margin:2px 4px 2px 0;' }, () => label)
        )
      )
    }
  },
...
function getOrgPathWithIds(tree: any[], id: string, path: { id: string, name: string }[] = []): { id: string, name: string }[] {
  for (const node of tree) {
