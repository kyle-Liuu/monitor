<template>
  ...
  <div class="streaminfo-page art-full-height">
    ...
    <ElCard shadow="never" class="art-table-card">
      <ArtTableHeader :columnList="columns" v-model:columns="columnChecks" @refresh="handleRefresh">
        <template #left>
          <div class="toolbar-left">
            ...
            <ArtExcelExport :data="tableData as any" :columns="exportColumns as any" filename="视频流数据" :auto-index="true"
      ...
      <ArtTable ref="tableRef" row-key="streamCode" :data="tableData" :loading="loading" :pagination="paginationState"
        :columns="columns" :layout="{ marginTop: 10 }" :table-config="{ emptyHeight: '360px' }"
    ...
    <ElDrawer v-model="drawerVisible" :title="drawerTitle" size="50%" :with-header="true" :before-close="handleClose">
      <ElForm ref="formRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <ElFormItem label="绑定组织" prop="orgId">
          <ElTreeSelect :key="treeSelectKey" v-model="formData.orgId" :data="orgTree" :props="{
            ...
            :render-after-expand="false" name="orgId">
        ...
        <ElFormItem label="算法标签">
          <div class="algo-tag-box" style="
            ...
            <ElTag v-for="algo in selectedAlgos" :key="algo" closable @close.stop="removeAlgo(algo)"
        ...
        <div v-for="algo in selectedAlgos" :key="algo" style="margin-bottom: 20px">
          <ElCard>
            ...
            <ElFormItem label="告警间隔">
              <ElInput v-model="algoConfigs[algo].interval" type="number" style="width: 120px" suffix="秒"
                :name="`algo-interval-${algo}`" />
            </ElFormItem>
            <ElFormItem label="告警窗口长度">
              <ElInput v-model="algoConfigs[algo].window" type="number" style="width: 120px" suffix="秒"
                :name="`algo-window-${algo}`" />
            </ElFormItem>
            <ElFormItem label="告警阈值">
              <ElInput v-model="algoConfigs[algo].threshold" type="number" style="width: 120px" suffix="秒"
      ...
      <ElDialog v-model="algoDialogVisible" title="选择算法" width="800px" append-to-body>
        <div class="algo-search-box" style="margin-bottom: 15px">
          <ElInput v-model="algoSearchKeyword" placeholder="搜索算法" clearable @input="filterAlgorithms"
        ...
        <ElTabs v-model="algoTab">
          <ElTabPane v-for="tab in algoTabs" :key="tab.name" :label="tab.label" :name="tab.name">
            <ElCheckboxGroup v-model="algoDialogChecked">
              <ElCheckbox v-for="item in filteredAlgoItemsByTab(tab.name)" :key="item.value" :label="item.label"
    ...
    <ElDialog v-model="calibrateDialogVisible" :title="calibrateDialogTitle" width="900px" top="20px" append-to-body
      :show-close="true">
      <div class="calibrate-dialog-content">
        ...
        <div class="calibrate-dialog-image" ref="imgBoxRef" @click="onImgBoxClick" @mousemove="onImgBoxMouseMove"
          ...
          <div v-if="!calibrateImgLoading && (!formData.streamCode || streamOffline)"
              ...
              <circle v-for="(p, pidx) in getPixelPoints(poly.normalizedPoints)" :key="'polypt-' + idx + '-' + pidx"
                ...
                :stroke="viewMode === 'edit'
                  ? selectedPolygonIdx === idx
                    ...
                    : '#0080ff'
                  : '#0080ff'
                  " stroke-width="2" style="cursor: move; pointer-events: auto;"
        ...
        <div v-if="calibrateMode === 'edit' && viewMode === 'edit'" class="calibrate-dialog-footer">
          <div class="calibrate-dialog-footer-left">
            ...
            <ElButton type="danger" size="small" style="margin-right: 10px" @click="handleClearAllPolygons">清除
            </ElButton>
            <ElButton type="danger" size="small" style="margin-right: 10px" @click="handleRemovePolygon">删除选中多边形
            </ElButton>
            <span v-if="drawing && !finished"
              style="margin-left: 12px; color: #ff8000; font-size: 14px; user-select: none">右键完成绘制</span>
          </div>
          <ElButton type="warning" size="small" @click="handleSaveAllPolygons">保存</ElButton>
        </div>
        <!-- 多边形命名弹窗 -->
        <ElDialog v-model="showNameInput" title="区域命名" width="300px" :close-on-click-modal="false" :show-close="false"
          destroy-on-close>
...
const allAlgoItems = computed(() => {
  return mockAlgoList.flatMap(tab => tab.items.map(item => ({
    ...item,
    tabName: tab.name
  })))
})
...
const filteredAlgoItems = computed(() => {
  ...
  return allAlgoItems.value.filter(item =>
...
function onImgBoxClick(e: MouseEvent) {
  ...
  // 检查图像是否已加载
  if (calibrateImgLoading.value || (!formData.streamCode && !hasImage.value) || (streamOffline.value && !hasImage.value)) {
