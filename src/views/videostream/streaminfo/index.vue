<template>
  <ArtTableFullScreen>
    <div class="streaminfo-page" id="table-full-screen">
        ...
        <ElForm ref="formRef" :model="formData" :rules="rules" label-width="100px">
          ...
          <ElFormItem label="绑定组织" prop="orgId">
            <ElTreeSelect :key="treeSelectKey" v-model="formData.orgId" :data="orgTree" :props="{
              ...
              style="width: 100%" :render-after-expand="false" name="orgId">
          ...
          <ElTabs v-model="algoTab">
            <ElTabPane v-for="tab in algoTabs" :key="tab.name" :label="tab.label" :name="tab.name">
              <ElCheckboxGroup v-model="algoDialogChecked">
                <ElCheckbox v-for="item in filteredAlgoItems" :key="item.value" :label="item.label" :value="item.value"
                  :id="item.id" v-show="item.tabName === tab.name">
                  {{ item.label }}
                </ElCheckbox>
              </ElCheckboxGroup>
            </ElTabPane>
      ...
      <ElDialog v-model="calibrateDialogVisible" width="900px" top="20px" append-to-body :show-close="true">
        <div class="calibrate-dialog-content">
          ...
          <!-- 多边形命名弹窗 -->
          <el-dialog v-model="showNameInput" title="多边形命名" width="300px" :close-on-click-modal="false"
            :show-close="false">
