...
<template>
  <div class="art-table" :class="tableClasses" :style="tableStyles">
    <div class="table-container">
      <ElTable ref="tableRef" v-loading="loading" :data="tableData" :row-key="mergedTableConfig.rowKey"
        :height="computedTableHeight" :max-height="mergedTableConfig.maxHeight"
        :show-header="mergedTableConfig.showHeader" :highlight-current-row="mergedTableConfig.highlightCurrentRow"
        :size="computedTableSize" :stripe="computedStripe" :border="computedBorder"
        :header-cell-style="computedHeaderCellStyle" :empty-text="mergedTableConfig.emptyText"
        :tree-props="mergedTableConfig.treeProps" :default-expand-all="mergedTableConfig.defaultExpandAll"
        :expand-row-keys="mergedTableConfig.expandRowKeys" :default-sort="mergedTableConfig.defaultSort"
        :tooltip-effect="mergedTableConfig.tooltipEffect" :show-summary="mergedTableConfig.showSummary"
        :sum-text="mergedTableConfig.sumText" :summary-method="mergedTableConfig.summaryMethod"
        :span-method="mergedTableConfig.spanMethod" :select-on-indeterminate="mergedTableConfig.selectOnIndeterminate"
        :indent="mergedTableConfig.indent" :lazy="mergedTableConfig.lazy" :load="mergedTableConfig.load"
        @row-click="handleRowClick" @selection-change="handleSelectionChange" @sort-change="handleSortChange"
        @filter-change="handleFilterChange" @current-change="handleCurrentRowChange" @header-click="handleHeaderClick"
        @header-contextmenu="handleHeaderContextmenu" @row-contextmenu="handleRowContextmenu"
        @row-dblclick="handleRowDblclick" @cell-click="handleCellClick" @cell-dblclick="handleCellDblclick"
        @cell-mouse-enter="handleCellMouseEnter" @cell-mouse-leave="handleCellMouseLeave"
        @expand-change="handleExpandChange" @select="handleSelect" @select-all="handleSelectAll">
        <!-- 序号列 -->
        <ElTableColumn v-if="shouldShowIndex" type="index" :width="INDEX_COLUMN_WIDTH" :label="INDEX_COLUMN_LABEL"
          ...
          <template v-for="col in visibleColumns" :key="getColumnKey(col)">
            ...
            <ElTableColumn v-else v-bind="getColumnProps(col)">
              ...
              <template v-if="col.useHeaderSlot" #header="headerSlotScope">
                <slot :name="col.headerSlotName || `${col.prop}-header`" v-bind="headerSlotScope" :prop="col.prop"
                  :label="col.label">
                  {{ col.label }}
                </slot>
              </template>
              ...
              <template v-if="col.useSlot" #default="slotScope">
                <slot :name="col.slotName || col.prop" v-bind="slotScope" :prop="col.prop"
    ...
    <div v-if="shouldShowPagination" class="table-pagination" :class="mergedPaginationConfig.align">
      <ElPagination v-model:current-page="currentPage" v-model:page-size="pageSize"
        :page-sizes="mergedPaginationConfig.sizes" :pager-count="isMobile ? MOBILE_PAGER_COUNT : DESKTOP_PAGER_COUNT"
        :total="total" :background="true" :size="mergedPaginationConfig.componentSize"
        :layout="computedPaginationLayout" :hide-on-single-page="mergedPaginationConfig.hideOnSinglePage"
...
/** 默认分页大小选项 */
const DEFAULT_PAGINATION_SIZES = [20, 30, 50, 100]
/** 默认顶部边距 */
