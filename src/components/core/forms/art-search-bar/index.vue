...
<template>
  <section class="search-bar art-custom-card" :class="{ collapsed: isCollapsed }">
    <!-- 折叠状态下只显示展开图标 -->
    <div v-if="isCollapsed" class="collapsed-toggle" @click="expandFully">
      <ElIcon>
        <Search />
      </ElIcon>
    </div>
    ...
    <div v-else class="search-content">
      <ElForm :model="filter" :label-position="labelPosition">
        <ElRow class="search-form-row" :gutter="gutter">
          <ElCol v-for="item in items" :key="item.prop" :xs="24" :sm="12" :md="item.elColSpan || elColSpan"
            :lg="item.elColSpan || elColSpan" :xl="item.elColSpan || elColSpan">
          ...
          <ElCol :xs="24" :sm="24" :md="elColSpan" :lg="elColSpan" :xl="elColSpan" class="action-column">
...
import { ArrowUpBold, ArrowDownBold, Search } from '@element-plus/icons-vue'
